...
export default function CinematicFaqPage() {
  return (
    <>
      ...
      <section className="border-t border-[#f0e0e4] bg-[#fff8f8] py-14 sm:py-20">
        <div className="mx-auto max-w-3xl px-6" style={{ animation: "fadeInUp 1s ease forwards", animationDelay: "0.3s", opacity: 0 }}>
          <Accordion type="single" collapsible className="w-full">
            {weddingConfig.faq.map((item, index) => (
              <AccordionItem key={index} value={`item-${index}`} className="border-b border-[#f0e0e4]">
                <AccordionTrigger className="py-6 text-left text-base font-light tracking-wide text-[#2c2424] hover:no-underline sm:text-lg" style={{ fontFamily: "var(--font-body)" }}>
                  {item.question}
                </AccordionTrigger>
                <AccordionContent className="pb-6 text-sm font-light leading-relaxed text-[#8a7f7f]">{item.answer}</AccordionContent>
              </AccordionItem>
            ))}
          </Accordion>
        </div>
